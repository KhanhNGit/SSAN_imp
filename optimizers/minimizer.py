...
class GACFAS:
    ...
    def proxy_gradients(self, index, input1, input2, labels, loss_func, UUID): 
        self.proxy[index].train()
        logits = self.proxy[index](input1, input2)[0]
        loss = loss_func(torch.cat([logits, logits]), labels, UUID)
